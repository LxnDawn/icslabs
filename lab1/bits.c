...
int abs(int x) 
{
	int isNegative = (x >> 31) & 1;	//most significant bit
	return (x ^ (x >> 31)) + isNegative;
}
/* 
...
int bitMask(int highbit, int lowbit) 
{
	...
	return ((mask << highbit << 1) ^ (mask << lowbit)) & (mask << lowbit);
}
/* 
...
int isLess(int x, int y) 
{
	...
	int notOverflow = x ^ ~y;
	int res = x + (~y + 1);
	return ((flag | (notOverflow & res)) >> 31) & 1;
}
/* 
...
{
	return (x >> 31) & 1;	//most significant bit
}
/* 
...
int isPower2(int x) 
{
	...
	return (!(x & (x + (~0x1 + 1))) & notZero & isPositive) | (0 & ~notZero) | (0 & ~isPositive);
}
/* 
...
int sum3(int x, int y, int z) 
{
	...
	***************************************************************/
	word1 = x ^ y ^ z;	//add by figures
	word2 = ((x & y) | ( y & z) | (x & z)) << 1;	//carry
  	/**************************************************************
